...
--For ui-lib
function handle_uihelp(cmdParams)
  add_to_chat(123,' ')
  add_to_chat(123,'UI Library Helper')
  add_to_chat(123,'-------------------')
  add_to_chat(123,'Available Options:')
  add_to_chat(123,'//gs c toggleui  --Toogle UI')
  add_to_chat(123,'//gs c runes     --RUN Rune Helper')
  add_to_chat(123,'//gs c strats    --Strategem Helper')
  add_to_chat(123,'//gs c beast     --BST Helper')
  add_to_chat(123,'//gs c speed     --Speed Info')
  add_to_chat(123,'//gs c timer     --Zone Timer')
  add_to_chat(123,'//gs c minui     --Minimal UI')
  add_to_chat(123,' ')
end

function handle_runehelper(cmdParams)
  if runeHelper == 'ON' then
    runeHelper = 'OFF'
    add_to_chat(123,'Rune Helper: [OFF]')
  else
    runeHelper = 'ON'
    add_to_chat(123,'Rune Helper: [ON]')
  end
end

function handle_strathelper(cmdParams)
  if strathelper == 'ON' then
    strathelper = 'OFF'
    add_to_chat(123,'Strat Helper: [OFF]')
  else
    strathelper = 'ON'
    add_to_chat(123,'Strat Helper: [ON]')
  end
end

function handle_toggleui(cmdParams)
  if gsmodeui == 'ON' then
    gsmodeui = 'OFF'
    add_to_chat(123,'GS Mode UI: [OFF]')
  else
    gsmodeui = 'ON'
    add_to_chat(123,'GS Mode UI: [ON]')
  end
end

function handle_bsthelper(cmdParams)
  if bsthelper == 'ON' then
    bsthelper = 'OFF'
    add_to_chat(123,'BST Helper: [OFF]')
  else
    bsthelper = 'ON'
    add_to_chat(123,'BST Helper: [ON]')
  end
end

function handle_speedinfo(cmdParams)
  if speedinfo == 'ON' then
    speedinfo = 'OFF'
    add_to_chat(123,'Toggle Speed Info: [OFF]')
  else
    speedinfo = 'ON'
    add_to_chat(123,'Toggle Speed Info: [ON]')
  end
end

function handle_zonetimer(cmdParams)
  if zonetimer == 'ON' then
    zonetimer = 'OFF'
    add_to_chat(123,'Zone Timer: [OFF]')
  else
    zonetimer = 'ON'
    add_to_chat(123,'Zone Timer: [ON]')
  end
end

function handle_minimalui(cmdParams)
  if minimal == 'ON' then
    minimal = 'OFF'
    add_to_chat(123,'Minimal UI: [OFF]')
  else
    minimal = 'ON'
    add_to_chat(123,'Minimal UI: [ON]')
  end
end
...
selfCommandMaps = {
    ...
    ['help']     = handle_help,
    ['test']     = handle_test,
    ['uihelp']   = handle_uihelp,
    ['runes']    = handle_runehelper,
    ['strats']   = handle_strathelper,
    ['toggleui'] = handle_toggleui,
    ['beast']    = handle_bsthelper,
    ['speed']    = handle_speedinfo,
    ['timer']    = handle_zonetimer,
    ['minui']    = handle_minimalui,
  }
