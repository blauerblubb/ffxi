...
user_settings = {
    user_spells = {
        Healing = T{'Cure', 'Cure II', 'Cura', 'Cure III'},
        Geomancy = T{'Indi-Voidance', 'Indi-Poison', 'Indi-Precision', 'Indi-Attunement', 'Indi-Focus'},
        Enhancing = T{'Protect', 'Shell', 'Haste', 'Aquaveil', 'Sneak', 'Invisible', 'Deodorize'},
        Ninjutsu = T{},
        Singing = T{'Victory March', 'Light Carol', 'Advancing March', 'Earth Carol'},
        Blue = T{},
		Summoning = T{'Carbuncle', 'Ifrit', 'Titan', 'Leviathan', 'Garuda', 'Shiva', 'Ramuh', 'Air Spirit', 'Fire Spirit', 'Ice Spirit', 'Earth Spirit', 'Water Spirit' }},
...
function precast(spell)
	...
	--composure
	if player.main_job == "RDM" and ability_usable(res.job_abilities[247]) and not buffactive[419] then
        if spell.en ~= "Composure" then
			cancel_spell()
			send_command('input /ja "'..res.job_abilities[247][gearswap.language]..'" <me>')
		end
        return
	end
	if player.main_job == "RDM" and spell_usable(res.spells[473]) and not buffactive[43] then
        if spell.en ~= "Refresh II" then
            cancel_spell()
            send_command('input /ma "'..res.spells[473][gearswap.language]..'" <me>')
        end
        return
	end
	if player.sub_job == "RDM" and spell_usable(res.spells[109]) and not buffactive[43] then
...
function ability_usable(ability)
    if windower.ffxi.get_ability_recasts()[ability.recast_id] == 0 then
        return true
	else
		return false
    end
